...
export default function StudioEditor() {
  ...
  useEffect(() => {
    try { const saved = localStorage.getItem(STORAGE_KEY); setPage(saved ? JSON.parse(saved) : defaultPageData); setState("saved"); }
  ...
  function save(status = page.status) {
    ...
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(payload)); setPage(payload); setState("saved"); setMessage(status === "published" ? "Published in this browser." : "Draft saved in this browser."); }
    catch { setState("error"); setMessage("Browser storage is full. Remove large media files and try again."); }
  }

  async function upload(file?: File) {
    if (!file) return;
    if (file.size > 10 * 1024 * 1024) { setMessage("For this frontend demo, choose a file smaller than 10 MB."); return; }
    setMessage("Preparing media…");
    try {
      const url = file.type.startsWith("image/") && file.size <= 2 * 1024 * 1024
      ...
      setMessage(file.type.startsWith("video/") || file.size > 2 * 1024 * 1024 ? "Media added for this browser session." : "Media added. Save your draft to keep it in this browser.");
    } catch {
      setMessage("The browser could not read this file. Try a smaller image or video.");
    } finally {
  ...
  return <main className="real-editor" style={{ "--accent": page.accent } as React.CSSProperties}>
    ...
    <div className="real-workspace">
      ...
      <section className="real-form"><header><p>PAGE CONTENT / {tab.toUpperCase()}</p><h1>{tabs.find(item => item.id === tab)?.label}</h1><span>{tab === "media" ? "Upload the work customers should see. Images and videos appear only on your public page." : "Changes appear in the preview as you type. Save when you are ready."}</span></header>{message && <div className="editor-message">{message}</div>}
        ...
        {tab === "story" && <div className="form-stack"><Field label="Story headline" value={page.storyTitle} onChange={v => update("storyTitle", v)} /><Field label="Business story" value={page.story} onChange={v => update("story", v)} textarea /><Field label="Hero image URL" value={page.heroImage} onChange={v => update("heroImage", v)} /><p className="field-help">You can paste an image URL here or upload a file under Photos & video and set it as the cover.</p></div>}
